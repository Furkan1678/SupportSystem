...
namespace SupportRequestManagement.Infrastructure.Data
{
    public class SupportRequestRepository : ISupportRequestRepository
    {
        ...
        {
            var sql = "SELECT * FROM support_requests WHERE id = @Id";
            return await _dbConnection.QuerySingleOrDefaultAsync<SupportRequestManagement.Domain.Entities.SupportRequest>(sql, new { Id = id });
        }

        public async Task<List<SupportRequestManagement.Domain.Entities.SupportRequest>> GetAllAsync()
        {
            var sql = "SELECT * FROM support_requests";
        ...
        {
            var sql = "SELECT * FROM support_requests WHERE user_id = @UserId";
        ...
        {
            var sql = "SELECT * FROM support_requests WHERE status = @Status";
        ...
        {
            var sql = "SELECT * FROM support_requests WHERE assigned_admin_id = @AdminId";
